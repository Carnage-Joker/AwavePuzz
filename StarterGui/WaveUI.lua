...
-- Wave Announcement Frame (big announcements - centered, scaled)
...
local function showAnnouncement(text, duration)
	...
	-- Use scaled target size (not hardcoded 400x80)
	local targetSize = UIScaleManager.scaleSize(400, 80, "menuElements")
	...
	task.delay(duration or 3, function()
		if thisId ~= currentAnnouncementId then return end
...
-- Wave Update
remoteEvents:WaitForChild("WaveUpdate").OnClientEvent:Connect(function(updateData)
	...
	if updateData.timeRemaining ~= nil then
		...
		timeLabel.TextColor3 = (remaining <= 30 and remaining > 0)
...
-- Game State Update
remoteEvents:WaitForChild("GameStateUpdate").OnClientEvent:Connect(function(stateData)
	if typeof(stateData) ~= "table" then return end
	...
	if state == "Victory" then
		...
		timeLabel.Text, zombieLabel.Text = "", ""
