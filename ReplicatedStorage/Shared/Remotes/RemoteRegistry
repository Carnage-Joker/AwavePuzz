...
local REMOTE_DEFINITIONS: { RemoteDef } = {
	...
	{Name = "RequestFunFact", Type = "Event"},
	{Name = "ShowFunFact", Type = "Event"},
	{Name = "UpdateFactStats", Type = "Event"},

	-- Voiceover system
	{Name = "PlayVoiceover", Type = "Event"},
	{Name = "StopVoiceover", Type = "Event"},

	-- Cure synthesis system
	{Name = "StartSynthesis", Type = "Event"},
	{Name = "SynthesisStateUpdate", Type = "Event"},
	{Name = "SynthesisPuzzleComplete", Type = "Event"},
	{Name = "SynthesisComplete", Type = "Event"},
	{Name = "SynthesisFailed", Type = "Event"},

	-- Notification system
	{Name = "ShowNotification", Type = "Event"},

	-- Portal matchmaking (additional remotes)
	{Name = "PortalQueueStatus", Type = "Event"},
	{Name = "PortalLeaveQueue", Type = "Event"},
	{Name = "PortalQueueJoined", Type = "Event"},
	{Name = "PortalQueueLeft", Type = "Event"},

	-- Betrayal system (additional remotes)
	{Name = "BetrayalOutcome", Type = "Event"},
	{Name = "BetrayalStatus", Type = "Event"},
}
...
-- ------------------------------------------------------------------
-- Safe FireClient wrapper (migrated from RemoteEventUtil)
-- ------------------------------------------------------------------

-- Safe FireClient wrapper
-- Validates player before calling :FireClient to avoid errors when player disconnects mid-update.
-- Usage: RemoteRegistry.SafeFireClient(remoteEvent, player, ...)
function RemoteRegistry.SafeFireClient(remoteEvent: RemoteEvent, player: Player, ...): boolean
	if typeof(remoteEvent) ~= "Instance" or not remoteEvent:IsA("RemoteEvent") then
		error(string.format("%s SafeFireClient: first argument must be a RemoteEvent", LOG_PREFIX))
	end
	...
	if not ok then
		warn(string.format("%s SafeFireClient failed for player %s: %s", LOG_PREFIX, tostring(player.Name), tostring(err)))
		return false
	end
