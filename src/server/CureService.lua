...
function CureService:handleDepositComponent(player, componentName)
        ...
        -- Ensure player has the component in inventory
        if self.playerManager and not self.playerManager:consumeInventoryItem(player, componentName, 1) then
                self.remoteEvents.CureUpdate:FireClient(player, {
                        type = "error",
                        message = "You do not have " .. componentName
                })
                return
        end
...
function CureService:handlePuzzleSolution(player, puzzleData)
        ...
        if isCorrect then
                ...
                if self.playerManager then
                        self.playerManager:addInventoryItem(player, randomComponent, 1)
                end
